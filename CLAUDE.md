...
- Distanziamento stesso cavaliere (BR-91): parametro dell'evento `draw_distance_target` (default 10, minimo di dominio 8 — vincolo a DB), degradazione a scala con warnings PUNTUALI, mai fallimento. Editor: `draw.reorder` (permutazione validata, flag di vicinanza ricalcolati) + `draw.suggest` ("sistema l'ordine": riparazione ANCORATA all'ordine corrente, minime modifiche, deterministica, mai un errore). **Editor v1 a frecce** (touch-friendly per il tablet in arena, E2E deterministico); il **drag&drop e il draw multi-giornata arrivano con l'allineamento a OrganizerRegia v4** — il prototipo resta la spec, il gap è dichiarato (backlog).
...
9. `AdminPanel.jsx` — admin di piattaforma v2 (uso interno TonettiMedia), solo desktop. Sidebar + barra comando ⌘K, home "Oggi" a triage con anzianità, pattern ispettore laterale (eventi/persone/run), correzione score nell'ispettore con motivo obbligatorio, registro a timeline solo-aggiunta. Principio di spec: "qui non esistono azioni silenziose" — ogni azione admin firmata, motivata, a registro append-only (BR-70..73). Implementazione non ancora in coda dev.
10. `PaginaPattern.jsx` — pagina pattern pubblica (per classe). Regola d'ingresso, sequenza numerata delle manovre da `reference/patterns.json`. Linkata da pagina evento, start list e app scribe. Diagrammi: mai le tavole NRHA (copyright) — si generano SVG originali dai dati di patterns.json, in stile PenRunner; in MVP bastano i passi testuali.
