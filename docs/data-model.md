...
**ScoreCard (sotto-ciclo, BR-27):** in compilazione → **chiusa** (annuncio: completezza validata, totale mostrato, sync come provvisorio) → **firmata** (batch **a ogni drag**, validato col giudice: il blocco di carte dal drag precedente, l'ultimo blocco chiude la classe; con tratto; riapertura tracciata solo pre-firma; un blocco saltato si firma al drag dopo — rituale proposto, mai imposto) → validata. Le bozze non lasciano mai il device; dalla firma la carta è immutabile: ogni modifica è una correzione BR-40 (snapshot prima/dopo in audit = versionamento, storia interrogabile per carta).
...
- **Payout** = vista derivata (`computePurse`/`selectPaybackBand`/`computePayout` in `packages/core`). **Montepremi** = iscrizioni (confermati × entry_fee, scratch inclusi) + added_money − trofei − 20% spese org. — formula **CONFERMATA da ART. 15 Reg. Disciplina Reining FISE/IRHA 2025** (verbatim in `reference/art15-montepremi.md`; regola <4 partecipanti e Payback A obbligatorio inclusi). *Residuo parametrico: la base del 20% (qui sole iscrizioni) — da precisare con la segreteria, insieme alla conferma Payback A = NRHA Schedule A. Validazioni leggere nel wizard (avvisi, mai blocchi): trofei > 75 € IVA compresa, quota classe > 30 € nei regionali. **Backlog dichiarato**: JACKPOT mode (% iscrizioni a scelta comitato, Fase 2), Judge Fee ≤5 € senza added (tipo quota non modellato), senza added money nessun obbligo di distribuzione (il payout resta calcolato e mostrato). **Fascia** dalla tabella ufficiale **Payback A** (`reference/payback-schedules.json`) per numero di cavalli iscritti. A premio **solo i piazzati eligibili** (BR-31: mai score_0/no_score); pari merito = premi delle posizioni coinvolte sommati e divisi; posizioni pagate senza eligibile → **non distribuite, restano allo show management**. Aritmetica in **centesimi interi** con quadratura garantita: `distribuito + non_distribuito = purse`, al centesimo. Multi-go (BR-34) è Fase 2: qui go singolo. Special events con Schedule B fuori MVP.
...
- **Turno stimato (ETA)** = vista derivata (`computeEta`): àncora = ultimo `started_at` osservato ("manda in campo"); senza àncora → modalità "da programma" (nessun orario promesso, solo conteggio run mancanti). Cadenza osservata (media mobile) sostituisce lo slot di default (BR-52); gli slot si contano sulle run effettive (lo scratch accorcia il tempo) ma i confini di drag sono FISSI sulla griglia del draw pubblicato (BR-51). Cascata sulle classi successive via `classes.scheduled_order`. Pause di programma: Fase 2. Regole BR-50..55.
